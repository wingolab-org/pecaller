...
use IO::Zlib;
use strict vars;
use vars qw(@fields %chr_num @sort_snp @ssort_snp $f $i $j %ins_a_count %chr_offset %ins_consensus %ins_needed %ins_needed_line @sample_names $dirname @lines @chr @pos @type);
...
while(<FILE>)
{
	...
	if( ($fields[5] eq "INS") || ($fields[5] eq "DENOVO_INS") )
	{
		...
		$ins_needed{$name} = 1;
		$ins_needed_line{$name} = $line_count;
	}
...
foreach my $i (keys %ins_needed)
{
	...
	{
		print "\n This is impossible.  We fail to find insertion $i at line $ins_needed_line{$i}\n";
		$bad_site{$ins_needed_line{$i}} = 1;
	}
	else
	{
		my $top = -50;
