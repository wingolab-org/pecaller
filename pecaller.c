...
int clean_config_probs(CNODE **cn, SAMNODE **sn, int n, int max,int max_gen,int indiv,int depth,int ref,int HAPLOID,double thres);
int sort_configs(const void *a,const void *b);
int sort_compare_sample_pointer(const void *a,const void *b);
int fill_config_probs(CNODE **cn, int n, SAMNODE **samples, int max,int indiv,int this_depth,int ref,int chrom,int HAPLOID,double thres);
...
void fill_sample_like(SAMNODE **samples,double **alpha,int max_gen,int indiv,SAMNODE **order,double norm,double avg);
  ...
  while(!ALL_FINISHED)
	...
	new_norm[i] = new_norm[i-1] * f_fact;
	  ...
	  for(iind=0;iind<INDIV;iind++)
	  {
		...
		total_configs = fill_config_probs(configs,total_configs,samples,max_gen,INDIV,ind,dom_int,chrom,HAPLOID,config_threshold);
	  	total_configs = clean_config_probs(configs,samples,total_configs,max_configs,max_gen,INDIV,INDIV-1,dom_int,HAPLOID,config_threshold);
	    }
	    else
	    {
...
void check_alpha_sanity(int **alpha_prior,int max_gen,int **first_alpha_prior,int ref,double **weight,int normal_factor)
{
  int i,j;
  int max;
...
/*---------------------------------------------------------------------*/
int clean_config_probs(CNODE **cn, SAMNODE **sn, int n, int max,int max_gen,int indiv,int depth,int ref,int HAPLOID,double ct)
{
  int i;
  ...
  for(i=0;i<n;i++)
  {
	...
	// printf("\n Last call was a %d \n",j);
      if(j < MAX_GENOTYPES)
      {
		...
		if(sn->dad)
			if(sn->mom) // Both
				old->no_denovo -= add_denovo(j,(int)old->sample_calls[sn->dad->which],(int)old->sample_calls[sn->mom->which],sn->sex,chrom);
			else // Dad Only
...
SAMNODE *sample_alloc()
{
	...
	if(!tn->kids) dump_error("Can't allocate space for 1000 kids");
