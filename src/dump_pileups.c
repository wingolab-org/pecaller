...
The code itself is Copyright (C) 2014, by David J. Cutler.
...
int clean_config_probs (CNODE ** cn, SAMNODE ** sn, int n, int max, int max_gen, int **alpha, double *coef, int indiv,
			int depth, int ref, int HAPLOID);
...
main (int argc, char *argv[])
{
  ...
  for (i = 0; i < INDIV; i++)
  {
    ...
    printf ("\n %d is sorted to %d which is .%s. .%s. and labeled as number %d", i, sorted_samples[i],
	    samples[i]->family, samples[i]->indiv, samples[i]->which);
  }
  ...
  while (!ALL_FINISHED)
	...
	for (ind = 0; ind < INDIV; ind++)
	  if (samples[ind]->tot > min_depth_needed && dom != 'N')
	  {
	    ...
	    total_configs =
	      fill_config_probs (configs, total_configs, samples[ind], max_gen, alpha_prior, coef_prior, INDIV, ind,
				 dom_int, chrom, HAPLOID);
	    // printf("\n About to clean config probs with total configs = %d\n\n",total_configs);
	    total_configs =
	      clean_config_probs (configs, samples, total_configs, max_configs, max_gen, alpha_prior, coef_prior, INDIV,
				  ind, dom_int, HAPLOID);
	...
	if (samples[ind]->tot > min_depth_needed && dom != 'N')
	{
	  ...
	  if (samples[ind]->final_p >= THRESHOLD)
	  {
	    if (samples[ind]->final_call != dom_int && issnp != MESS)
	    {
	      ...
	      if (issnp == REF_ALLELE)
		issnp = this_site;
	      else if (issnp != this_site)
	      {
		if (this_site <= INSERTION && issnp <= INSERTION)
		  issnp = MESS;
		else if (issnp > INSERTION && this_site <= INSERTION)
		  issnp = this_site;
		else if (issnp > INSERTION && this_site > INSERTION)
		  issnp = MESS;
	      }
...
/*---------------------------------------------------------------------*/
int
get_snp_type (SAMNODE * sm, int ref, char *minor)
{
  int a, b;
...
/*---------------------------------------------------------------------*/
int
clean_config_probs (CNODE ** cn, SAMNODE ** sn, int n, int max, int max_gen, int **alpha, double *coef, int indiv,
		    int depth, int ref, int HAPLOID)
{
  int i;
...
/*---------------------------------------------------------------------*/
void
fill_hom_like (CNODE * cn, SAMNODE ** sn, int n, int **alpha, double *coef)
{
  int i, j, ii;
  double prob;
...
int
fill_config_probs (CNODE ** cn, int n, SAMNODE * sn, int max, int **alpha, double *coef, int indiv, int this_depth,
		   int ref, int chrom, int HAPLOID)
{
