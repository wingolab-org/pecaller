...
main (int argc, char *argv[])
{
    ...
    sprintf(sss, "%s.map", argv[4]);
    if ((mapfile = fopen (sss, "w")) == (FILE *) NULL)
    {
        printf ("\n Can not open file %s for writing\n", sss);
        exit (1);
    }
    
    sprintf(sss, "%s.ped", argv[4]);
    if ((pedfile = fopen (sss, "w")) == (FILE *) NULL)
    {
        printf ("\n Can not open file %s for writing\n", sss);
        exit (1);
    }
