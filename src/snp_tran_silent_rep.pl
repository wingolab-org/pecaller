...
use strict 'vars';
use vars qw(@fields $site_code $snp_id %tran_map %snp_type @filehandles %rs %rs_type %ts %tv %transition %transver @files $i $j $last @samples);
...
{
	print "\t$i\_transistions\t$i\_transversions\t$i\_ratio\t$i\_in_dbsnp\t$i\_dbsnp_ratio";
}
for($i=0;$i<@samples;$i++)
{
	print "\n$samples[$i]";
