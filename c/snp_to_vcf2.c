...
int make_allele_list(char *start, char **list);
void make_allele_count(char *start, int *count);
...
int main(int argc, char *argv[])
{
	...
	printf("##INFO=<ID=AC,Number=.,Type=Integer,Description=\"Allele Count\">\n");
	...
	char **allele_list;
	int *allele_count;
	allele_list = cmatrix(0, 100, 0, 1000);
	...
	while (len > 5)
	{
		...
		tot_a = make_allele_list(token, allele_list);
		token = strtok(NULL, "\n\t ");
		make_allele_count(token, allele_count);
		int AN = 0;
		j = 0;
		for (i = 0; i < tot_a; i++)
		{
			AN += allele_count[i];
			if (allele_list[i][0] != ref)
			{
				if (j == 0)
		...
		if (!drop_it)
		{
			...
			printf("\n%s\t%d\t.\t%s\t%s\t.\t%s\t%s;NS=%d\tGT:GQ",
				   chrom, pos, ref_string, alt_a_final, slabel, alt_count_final, tot_samples);
			char *token2;
			double op;
			for (i = 0; i < tot_samples; i++)
			{
				...
				if (op >= min_prob)
					...
					printf("\t./.");
				printf(":%d", qual_to_phred(atof(token2)));
			}
		}
...
int make_allele_list(char *start, char **list)
{
	...
	while (*s != '\0')
	{
		...
		s++;
		// printf("\n In here with this = %d which = %d",this,which);
	}
	list[this][which] = '\0';
	return this + 1;
}
...
void make_allele_count(char *start, int *count)
{
	...
	while (*s != '\0')
	{
		...
		{
			temp[which] = '\0';
			count[this ++] = atoi(temp);
			which = 0;
		}
		s++;
		// printf("\n In here 2 with this = %d which = %d",this,which);
	}
	temp[which] = '\0';
	count[this ++] = atoi(temp);
	return;
